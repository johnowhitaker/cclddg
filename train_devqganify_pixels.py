...
def train(args):
    ...
    # Create the model
    unet = UNet(image_channels=3*2, n_channels=args.n_channels_unet,
               z_dim=args.z_dim, n_z_channels=args.n_z_channels,
               ).to(device) # 4 or 8 or whatever based on ae
    disc = Discriminator(image_channels=3*3, n_channels=args.n_channels_disc).to(device)
        ...
        while t[0] > 0:
            ...
            if t[0]==0:
                for s in range(n_examples):
                    im_out.paste(ddg_context.tensor_to_image(pred_im[s]), (img_size*s, img_size*2))
        return im_out
    ...
    for i in tqdm(range(0, args.n_batches)): # Run through the dataset
        ...
        gen_pred_x0 = unet(gen_input, t, c, z)[:,:3,:,:] # Run xt through the network to get its predictions
